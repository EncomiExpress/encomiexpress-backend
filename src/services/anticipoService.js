const { AnticipoExcedente, Conductor, SalidaProgramada, SalidaVehiculoConductor, Vehiculo, Ruta, Destino, Usuario, sequelize } = require('../models');
...
const validarFechaEntregaAnticipo = (fechaEntrega, salida) => {
  if (!fechaEntrega) return;
  if (salida?.fechaSalida && fechaEntrega > salida.fechaSalida) {
    throw new AppError(`La fecha de entrega no puede ser posterior a la salida de la ruta (máximo el ${salida.fechaSalida})`, 400);
  }
...
const getAll = async ({ idConductor, idSalida, estado, habilitado, anio, mes, q, page = 1, limit = 10, sortBy } = {}) => {
  try {
    const where = {};
...
// El vehículo de un anticipo es el que le tocó a SU conductor en esa salida (una
// salida puede repartirse entre varios vehículos+conductor) — ya no hay una
// asociación directa SalidaProgramada→Vehiculo, así que se resuelve aparte contra la
// tabla intermedia y se anexa en salida.vehiculo para no romper la forma que ya
// esperaba el frontend.
//
// De paso, esta misma búsqueda dice si el anticipo quedó "huérfano": si no aparece
// ningún par activo con ese idSalida+idConductor es porque el par se reasignó a otro
...
const attachVehiculo = async (anticipo) => {
  let par = null;
  if (anticipo?.salida) {
    par = await SalidaVehiculoConductor.findOne({
      where: { idSalida: anticipo.idSalida, idConductor: anticipo.idConductor, habilitado: true },
      include: [{ model: Vehiculo, as: 'vehiculo' }],
    });
    anticipo.salida.dataValues.vehiculo = par?.vehiculo || null;
  }
  anticipo.dataValues.esHuerfano = !!anticipo?.salida && !par;
  return anticipo;
};
...
const create = async (data) => {
  const { idSalida, idSalidaVehiculoConductor, valorAnticipo, soporte, fechaEntrega } = data;

  if (!idSalida) {
    throw new AppError('La ruta es obligatoria', 400);
  }
  if (!idSalidaVehiculoConductor) {
  ...
  // Una salida ahora puede tener varios pares vehículo+conductor — hay que confirmar
  // que el par elegido de verdad pertenece a esta salida, y de ahí sale el conductor
  // (ya no se selecciona aparte, así nunca queda desincronizado).
  const par = await SalidaVehiculoConductor.findOne({ where: { idSalidaVehiculoConductor, habilitado: true } });
  if (!par || par.idSalida !== salida.idSalida) {
  ...
  // La regla es "un anticipo activo por conductor dentro de la salida" — con varios
  // conductores por salida, cada uno puede tener el suyo sin chocar entre sí. Ya no
  // hace falta lógica adicional para "corredores reutilizados" (cada salida es una
  // fila distinta ahora, no una plantilla que se reutilizaba fecha tras fecha).
  const anticipoExistente = await AnticipoExcedente.findOne({
    where: { idSalida: salida.idSalida, idConductor, habilitado: true, estado: { [Op.in]: ['Entregado', 'En Legalización'] } }
  });
  ...
  validarFechaEntregaAnticipo(fechaEntregaLimpia, salida);
...
const update = async (id, data) => {
  ...
  if (enLegalizacion) {
    if (idSalida !== undefined || idSalidaVehiculoConductor !== undefined || valorAnticipo !== undefined || fechaEntrega !== undefined) {
    ...
    // Comprobante obligatorio para legalizar -- mismo criterio que la evidencia de
    // entrega final de Paquetes (ver LOGICA.md, "Evidencia de entrega final
    ...
    // anticipo.estado === 'Entregado': la ruta todavía no arrancó, así que todavía no
    // hay nada que legalizar.
  ...
  let salidaResuelta;
  if (idSalida !== undefined || idSalidaVehiculoConductor !== undefined) {
    if (!idSalida || !idSalidaVehiculoConductor) {
    ...
    salidaResuelta = salida;
    // El conductor del anticipo siempre sigue al del par vehículo+conductor elegido
    // (una salida puede tener varios) — no se selecciona aparte, para que nunca
    ...
    const conductorDeSalida = await Conductor.findByPk(par.idConductor);
    if (conductorDeSalida && !tieneLicenciaVigente(conductorDeSalida.categoriasLicencia)) {
      throw new AppError('El conductor de esta ruta tiene la licencia de conducción vencida y no puede recibir anticipos', 400);
    }
    idSalidaFinal = salida.idSalida;
  ...
  if (cleanedFechaEntrega !== undefined) {
    // Si esta misma petición no trae idSalida (raro -- el frontend siempre manda
    // ambos mientras el anticipo sigue editable, ver ActualizarAnticipoExcedente.jsx),
    // se valida contra la salida que el anticipo ya tenía.
    const salidaParaValidar = salidaResuelta || await SalidaProgramada.findByPk(anticipo.idSalida);
    validarFechaEntregaAnticipo(cleanedFechaEntrega, salidaParaValidar);
  }
  ...
  let newExcedente;
  // A qué salida apunta el auto-completado de más abajo -- ver ahí. Se resuelve acá
  // (no en el momento de usarlo) porque `esIda`/`salidaDelAnticipo` viven dentro del
  // bloque de abajo y quedan fuera de alcance después.
  let idSalidaParaAutoCompletar;
  ...
  if (valorGastado !== undefined) {
    ...
    const esIda = salidaDelAnticipo?.idSalidaIda == null;
    // El anticipo vive en la ida, pero lo que de verdad puede necesitar
    // auto-completarse acá es el REGRESO (la ida normalmente ya está Completada
    ...
    if (!rutaCancelada) {
      if (esIda && !salidaDelAnticipo.salidaRegreso) {
        throw new AppError(
    ...
    // El gasto puede superar lo entregado — queda un excedente negativo (la empresa
    // le debe reponer la diferencia al conductor) en vez de bloquearlo. `excedente`
    // positivo = a favor de la empresa, negativo = a favor del conductor; en ambos
    // casos queda "Excedente pendiente" hasta que se confirme el cierre
    // (entregarExcedente) — solo si da exactamente 0 no queda nada por resolver y
    // pasa directo a Completado.
  ...
  await anticipo.update({
    idSalida: idSalidaFinal !== undefined ? idSalidaFinal : anticipo.idSalida,
  ...
  // cuando de verdad ya volvió. `idSalidaParaAutoCompletar` (calculado arriba) ya
  // resuelve el id correcto. Sigue siendo best-effort/seguro para un regreso CON
  // carga real: para ese caso dejarPaquetesEnSede ya lo completó antes de que la
  // legalización llegara siquiera a ser posible (exige sedes completas), así que acá
  // `intentarAutoCompletar` encuentra `estado !== 'En Ruta'` y no hace nada -- esto
  // solo actúa de verdad cuando nada más lo hizo antes. require lazy para no atar el
  // orden de carga. Ver LOGICA.md.
  if (autoEstado) {
    await require('./salidaProgramadaService').intentarAutoCompletar(idSalidaParaAutoCompletar || anticipo.idSalida);
  }
...
const entregarExcedente = async (id, { soporte }) => {
  ...
  await anticipo.update({
    estado: 'Completado',
    // `soporte` es un array — si llega uno nuevo se agrega a los que ya había, no
    // los reemplaza.
...
// Caso real distinto del "huérfano" (ver LOGICA.md, "Anticipos huérfanos al
// reasignar conductor"): el admin registró un anticipo pensando en dárselo a un
// conductor, pero al final no se lo dio, y para cuando se dio cuenta la ruta ya
// había arrancado (el anticipo ya pasó a "En Legalización"). Nunca va a llegar a
// "Completado" -- eso depende de que el conductor reporte valorGastado desde el
// móvil, algo absurdo de pedirle si nunca recibió esa plata. Fusionado dentro de
// toggleHabilitado() (ver ahí) en vez de ser una acción/botón aparte -- decisión de
// la usuaria (2026-09-13): un solo botón ("Inhabilitar"), no dos. Solo aplica a
// Entregado/En Legalización -- "Excedente pendiente" ya implica un valorGastado
// real reportado (sí hubo plata de por medio), eso sigue bloqueado hasta resolverse
// por "Confirmar devolución/reposición". Ver LOGICA.md, "Cerrar un anticipo que
// nunca se llegó a entregar".
const ESTADOS_CERRABLES_SIN_ENTREGA = ['Entregado', 'En Legalización'];
const MOTIVO_CIERRE_MAX_LENGTH = 500;

// fileUrls: array de URLs recién subidas a Cloudinary — se agregan a las que ya
// tenía el anticipo (nunca se pisan las anteriores).
...
const toggleHabilitado = async (id, { motivo } = {}) => {
  ...
  if (anticipo.habilitado === true) {
    // Huérfano: el conductor de este anticipo ya no es par activo de su salida (el
    // par se reasignó a alguien más después de entregarle este anticipo) — nada
    // del flujo normal (legalizar desde el móvil, cerrar la ruta) va a llegar a
    // tocarlo nunca, así que se deja inhabilitar directo sin exigir "Completado".
    // Decisión de la usuaria (2026-09-12): solo para este caso puntual (ver
    // LOGICA.md, "Anticipos huérfanos al reasignar conductor").
    const parVigente = await SalidaVehiculoConductor.findOne({
      where: { idSalida: anticipo.idSalida, idConductor: anticipo.idConductor, habilitado: true },
    });
