...
-- estado, convoy ni paradas: eso vive en `salida_programada`, una fila por cada
...
-- Paradas intermedias del corredor de un PAR vehículo+conductor de una salida
-- (municipios donde ESE vehículo deja paquetes en el camino, ej. Medellín →
-- Yarumal → Valdivia → Tarazá → Caucasia) — información aditiva, no reemplaza a
-- salida_programada.id_destino heredado de la ruta (que sigue siendo el destino
-- final del tramo troncal, compartido por todos los pares de la salida). Son
-- propias de CADA par, no de la salida completa: ej. ruta fraccionada
-- Medellín → Rionegro donde un vehículo del convoy pasa por Guarne y sigue a
-- Rionegro, mientras otro vehículo del mismo convoy va directo a Rionegro sin
-- pasar por Guarne. Dos pares (de la misma salida o de salidas distintas)
-- pueden compartir una misma parada sin problema.
CREATE TABLE salida_parada (
  id_salida_parada             INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- Un mismo municipio no puede repetirse dos veces como parada del recorrido de UN
-- mismo par vehículo+conductor, ni dos paradas de ESE par pueden compartir la misma
-- posición — dos pares distintos (misma salida u otra) SÍ pueden compartir una
-- misma parada, cada uno va por separado.
CREATE UNIQUE INDEX uq_parada_par_destino ON salida_parada (id_salida_vehiculo_conductor, id_destino);
CREATE UNIQUE INDEX uq_parada_par_orden ON salida_parada (id_salida_vehiculo_conductor, orden);
